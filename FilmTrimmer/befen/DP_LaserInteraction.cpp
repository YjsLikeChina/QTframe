...
bool DP_LaserInteraction::ConnectLaser(bool bComputerCard /*= true*/)
{
	bool bRes = false;
	if (bComputerCard)
	{
		if(!m_pLaserCtrl->m_bLoadSucess)
			bRes = m_pLaserCtrl->InitDll();
	}
	else 
	{
		for (int i = 0; i < 2; i++)
...
bool DP_LaserInteraction::UploadLaserData(ST_LASER_DATA stLaser, bool bComputerCard /*= true*/)
{
	bool bRes = false;
	if (bComputerCard)
	{
		bRes = LaserCardSeparate(m_stLaserData);
	}
	else
	{
		bRes = LaserCardInPC(m_stLaserData);
	}
	return bRes;
}
...
bool DP_LaserInteraction::LaserCardSeparate(ST_LASER_DATA stLaser)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (m_bEnableLaser[i] && m_pLaserCtrl->m_bLoadSucess)
		{
			if (m_pLaserCtrl->HMSGetPraStatus(i) <= 0)
			{
				emit SigLaserResultMsg(QString::fromLocal8Bit("激光状态未连接"));
				bResult = FALSE;
				break;
			}
			if (stLaser.strLaserFilePath.IsEmpty())
			{
				...
				if (bFindFile)
				{
					...
					bResult = m_pLaserCtrl->HMSOpenFile(strFileA.GetBuffer());
			...
			bResult = m_pLaserCtrl->HMSSetPoleInfo(stLaser.nLaserStep, strListDist.GetBuffer());
			strListDist.ReleaseBuffer();
			PRINTF(_T("Call laser function:HMSSetPoleInfo,to laser end,result%d."), bResult);
			if (!bResult)
				break;

			//PRINTF(_T("Call laser function:HMSSetPoleSize(%.3f,%.3f),to laser start..."), stLaser.dlEarWidth, stLaser.dlEarHeight);
			//bResult = m_pLaserCtrl->HMSSetPoleSize(stLaser.dlEarWidth, stLaser.dlEarHeight);
			//PRINTF(_T("Call laser function:HMSSetPoleSize,to laser end,result%d."), bResult);
			//if (!bResult)
			//break;
			...
			if (i == 0)
			{
				...
				bResult = m_pLaserCtrl->HMSSetPoleSizeByCard(i, stLaser.dlEarWidth, stLaser.dlEarHeight);
				...
				//bResult = m_pLaserCtrl->HMSSetPower(int(stLaser.dlLaser1Power + 0.00001));
				bResult = m_pLaserCtrl->HMSSetPowerByCard(i, int(stLaser.dlLaser1Power + 0.00001));
				...
				//bResult = m_pLaserCtrl->HMSSetFreq(int((stLaser.dlLaser1Freq + 0.00001)*KKHZ));
				bResult = m_pLaserCtrl->HMSSetFreqByCard(i, int((stLaser.dlLaser1Freq + 0.00001)*KKHZ));
			...
			else
			{
				...
				//bResult = m_pLaserCtrl->HMSSetPower(int(stLaser.dlLaser2Power + 0.00001));
				bResult = m_pLaserCtrl->HMSSetPowerByCard(i, int(stLaser.dlLaser2Power + 0.00001));
				...
				//bResult = m_pLaserCtrl->HMSSetFreq(int((stLaser.dlLaser2Freq + 0.00001)*KKHZ));
				bResult = m_pLaserCtrl->HMSSetFreqByCard(i, int((stLaser.dlLaser2Freq + 0.00001)*KKHZ));
				PRINTF(_T("Current Laser Number %d"), i);
			...
			bResult = m_pLaserCtrl->UploadMarkDate(i, 0);
