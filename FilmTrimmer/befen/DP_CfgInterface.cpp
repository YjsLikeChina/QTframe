...
bool DP_CfgInterface::initCfgValue(QVector<ST_MODULE>& VMoudel, QString path)
{
	...
	for (int i = 0; i < Moudul.GetSize(); i++)
	{
		...
		for (j = 0;; j++)
		{
			...
			else if (cstrOtherAttribute == _T("Updata"))
			{
...
bool DP_CfgInterface::WriteValue(const QMap<QString, QString>& ModifyVal, QString qstrCfgPath)
{
	...
	for (int i = 0; i < nLength; i++)
	{
		...
		if (!WriteStringToFile(cstrChildKey, _T("Vel"), cstrChildVal, cstrFilePath))
		{
			return false;
		}
