...
void UI_HistoricalInformation::initUI()
{
	...
	ui.TW_HistoryInfo->setEditTriggers(QAbstractItemView::NoEditTriggers);
}

void UI_HistoricalInformation::initConnect()
{
	connect(ui.CB_HistoryInfoSel, SIGNAL(activated(int)), this, SIGNAL(SlotSwitchSection(int)));
...
void UI_HistoricalInformation::SlotSwitchSection(int nIndex)
{
	...
	//发送信号到业务层
	emit SigSwitchSection(nIndex,m_qMapHistoryInfoPath.value(nIndex));
}
...
{
	SlotSwitchSection(m_nLastIndex);
}
